...
const _CardInDetail: FC<Props> = React.memo((props: Props) => {
    ...
    return (
        <Modal
            visible={visible}
            title={thisIsCreating ? '新建一个卡片' : '编辑'}
            okText={thisIsCreating ? '创建' : '保存'}
            onCancel={onCancel}
                ...
                <Form layout="vertical" onKeyDown={keydownEventHandler}>
                    <Form.Item label="名称">
                        {getFieldDecorator('title', {
                            rules: [{ required: true, message: '请输入看板的名字！' }],
                    ...
                    {thisIsCreating ? undefined : (
                        <Row>
                            <Popconfirm title={'你确定?'} onConfirm={onDelete}>
